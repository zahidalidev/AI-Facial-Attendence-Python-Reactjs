...
class AttendanceCode(Resource):
    ...
    def get(id):
        try:
            ...
            return 'This attendance_code is already existed'

        except Exception:
            return traceback.format_exc()

    # validating attendance code for attendance
    @staticmethod
    def post():
        try:
            data = request.json
            ...
            # find attendance_code to verify attendance_code with the same course ID and date already exists or not
            validate_attendance_code = attendanceCodeCol.find_one(
                {"courseID": ObjectId(course_id), "date": today_date, "attendanceCode": attendanceCode})

            # if attendance_code is not exists
            if validate_attendance_code is None:
                return "Invalid course id or code has been expired"
    ...
    def delete(id):
        try:
            ...
            if attendance_code is None:
                return 'attendance Code id is invalid'
